...
class NeuralNet:
    ...
    def __init__(self, feature_matrix, distance_matrix, hidden_nodes=15, k=40):
        self.k = k
        ...
        self.input_dim = k * data_dim  # new input dimension

        # W1 now takes flattened neighbor matrix as input
        self.W1 = np.random.randn(self.input_dim, hidden_nodes) * np.sqrt(1 / self.input_dim)
    ...
    def create_input_layer(neighbors_inds, feature_matrix):
        """
        Build the input vector by flattening the raw k x m neighbor matrix.
        """
        neighbors_matrix = feature_matrix[neighbors_inds, :]  # shape: (k, m)
        return neighbors_matrix.flatten()  # shape: (k*m,)
    ...
    def forward_prop(self, user_ind):
        ...
        # Find k nearest neighbors of user
    ...
    def train(self, epochs=200, tol=1e-3, valid_indices=None):
        ...
        for epoch in range(epochs):
            ...
            # Show progress at the start (to see gradient descent), then every 10 epochs
    ...
    def predict(self, user_ind, feature_matrix, distance_matrix, valid_indices=None):
