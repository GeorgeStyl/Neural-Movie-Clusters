...
min_clusters = 3
max_clusters = 50
n_iterations = 5  # Number of runs per cluster count
...
for n_clusters in cluster_range:
    ...
    for run in range(n_iterations):
        ...
        # Fit KMeans — pass log_file only on the very first instantiation
        kmeans = CustomKMeans(
            n_clusters=n_clusters,
            metric=metric,
            log_file=log_file if (n_clusters == min_clusters and run == 0) else None
        )
        labels = kmeans.fit(X, max_iters=60, tol=1e-6)
...
plt.tight_layout()
plt.savefig(PLOTS_DIR /f"db_index_{metric}_L{min_clusters}-{max_clusters}_runs{n_iterations}.png")
plt.show()
